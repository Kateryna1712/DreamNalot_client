...
const SignUp = () => {
    ...
    const onSubmit: SubmitHandler<IFormSignUp> = async (data) => {
        try {
            await axios.post('http://localhost:3000/mail/save_user_mail', {
                ...data
            })
            reset()
        } catch (error) {
    ...
    return (
        <div className={styles.sign__up_container}>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className={styles.email__form}>
                <div className={errors.email ? styles.email__form_input_error : styles.email__form_input}>
                    <input
                        {...register('email', {
                        ...
                        placeholder='Email'
                        type="email" />
                    <button className={styles.button} type='submit'><img loading='lazy' src={confirm_icon} alt="" /></button>
                </div>
                <span className={styles.error}>{errors.email?.message}</span>
            </form>
            <section className={styles.sign__up_text}>
                By signing up, you want to receive RUNGE emails. Please see out <span><a className={styles.links} href="">Term & Conditions</a></span> and <span><a className={styles.links} href="">Privacy Policy</a></span> for more details.
            </section>
        </div>
    );
};
